...
//        private AuctionStatus auctionStatus;
    ...
    public static GoodsCard from(Goods goods) {
        return GoodsCard.builder()
                .goodsId(goods.getId())
                .sellerId(goods.getSellerId())
                .sellerNickname(null)        // 추천 API에서는 미사용
                .wishCount(0)                // 기본값
                .checkWish(false)            // 기본값
//                .category(goods.getCategory())
                .mainFilePath(null)          // Goods에 없음 → null
                .title(goods.getTitle())
                .animeTitle(null)            // Goods에 없음 → null
                .currentBidAmount(null)      // Goods에 없음 → null
